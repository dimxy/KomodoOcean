...
static bool ActivateBestChainStep(bool fSkipdpow, CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    int32_t notarizedht,prevMoMheight; uint256 notarizedhash,txid;
    notarizedht = komodo_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    std::cerr << __func__ << " fSkipdpow=" << fSkipdpow << " notarizedht=" << notarizedht << " pindexOldTip->nHeight=" << (pindexOldTip ? pindexOldTip->nHeight : 0) << " pindexFork=" << pindexFork << " pindexFork->nHeight=" << (pindexFork ? pindexFork->nHeight : 0) << std::endl;
